...
class AttentionRecognitionHead(nn.Module):
  ...
  def beam_search(self, x, beam_width, eos):
    ...
    for i in range(self.max_len_labels):
      ...
      state = state.index_select(1, predecessors.squeeze().int())
    ...
    while t >= 0:
      ...
      t_predecessors = stored_predecessors[t].index_select(0, t_predecessors).squeeze().int() #  TODO: evaluate conversion to int 
